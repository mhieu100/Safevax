...
public interface VaccineRepository extends JpaRepository<Vaccine, Long>, JpaSpecificationExecutor<Vaccine> {
       ...
       @Query("SELECT DISTINCT manufacturer FROM Vaccine WHERE manufacturer IS NOT NULL")
       List<String> findDistinctManufacturers();

       @Query("SELECT DISTINCT dosesRequired FROM Vaccine")
       List<Integer> findDistinctDosesRequired();

       @Query("SELECT DISTINCT disease FROM Vaccine WHERE disease IS NOT NULL")
